  ...
  return (
    <article className={`pb-6 sm:pb-8 ${
      ...
      <div className={`flex flex-col ${article.image ? 'md:flex-row md:gap-6' : ''}`}>
        ...
        {/* Image - Right Side */}
        {article.image && (
          <div className="mt-4 md:mt-0 md:order-2 md:w-[320px] lg:w-[400px] xl:w-[450px] flex-shrink-0 overflow-hidden rounded-lg">
            <Link href={`/article/${article.slug}`}>
              <Image 
                ...
                width={420} 
                height={260} 
                className="w-full h-48 md:h-56 lg:h-60 object-contain rounded-lg transition-transform duration-300" 
                unoptimized
              />
            </Link>
          </div>
